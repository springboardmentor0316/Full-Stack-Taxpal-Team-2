...
transporter.verify((error, success) => {
  if (error) {
    console.log("[v0] EMAIL CONFIG ERROR - Please check your .env file:")
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("[v0] EMAIL SENDING FAILED:", emailError.message)
      console.log("[v0] Check if EMAIL_USER and EMAIL_PASSWORD are set in .env")
      return res.status(500).json({ 
        message: "Failed to send verification code. Check email configuration.", 
...
exports.resendCode = async (req, res) => {
  try {
    ...
    try {
      ...
      res.json({ message: "Verification code resent to your email", success: true })
    } catch (emailError) {
      console.log("[v0] EMAIL SENDING FAILED:", emailError.message)
      return res.status(500).json({ 
